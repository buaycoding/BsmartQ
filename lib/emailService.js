...
function createEmailService({ dbPool, transport } = {}) {
  const resolvedDbPool = dbPool || null;
  const defaultFrom = process.env.RESEND_FROM || process.env.SMTP_FROM || process.env.SENDGRID_FROM || 'BsmartQ <no-reply@bsmartq.app>';
  let resendClient = null;
  ...
  async function sendEmail({ to, subject, text, html, category = 'general', metadata = {}, retries = 2 }) {
    ...
    const fromAddress = process.env.RESEND_FROM || process.env.SMTP_FROM || process.env.SENDGRID_FROM || defaultFrom;
    ...
    for (let attempt = 1; attempt <= retries + 1; attempt += 1) {
      try {
        ...
        if (typeof transport === 'function') {
          ...
          response = await resendClient.emails.send(payload);
        } else {
